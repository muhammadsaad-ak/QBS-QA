...
import {
    QbsNavigationItem,
    QbsVerticalNavigationAppearance,
    QbsVerticalNavigationMode,
    QbsVerticalNavigationPosition,
} from '../navigation.types';
import { QbsVerticalNavigationAsideItemComponent } from './components/aside/aside.component';
...
import { QbsVerticalNavigationGroupItemComponent } from './components/group/group.component';
import { QbsVerticalNavigationSpacerItemComponent } from './components/spacer/spacer.component';
import { QbsScrollbarDirective } from '../../../directives/scrollbar/scrollbar.directive';
...
@Component({
    selector: 'qbs-vertical-navigation',
    templateUrl: './vertical.component.html',
    styleUrls: ['./vertical.component.scss'],
    animations: qbsAnimations,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs: 'qbsVerticalNavigation',
    standalone: true,
    imports: [
        QbsScrollbarDirective,
        QbsVerticalNavigationAsideItemComponent,
        ...
        QbsVerticalNavigationGroupItemComponent,
        QbsVerticalNavigationSpacerItemComponent,
    ],
})
export class QbsVerticalNavigationComponent
    implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: QbsVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: QbsVerticalNavigationMode = 'side';
    @Input() name: string = this._qbsUtilsService.randomId();
    @Input() navigation: QbsNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: QbsVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output()
    readonly appearanceChanged: EventEmitter<QbsVerticalNavigationAppearance> =
        new EventEmitter<QbsVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<QbsVerticalNavigationMode> =
        new EventEmitter<QbsVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> =
        new EventEmitter<boolean>();
    @Output()
    readonly positionChanged: EventEmitter<QbsVerticalNavigationPosition> =
        new EventEmitter<QbsVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<QbsNavigationItem> =
        new ReplaySubject<QbsNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<QbsNavigationItem> =
        new ReplaySubject<QbsNavigationItem>(1);
    ...
    private _qbsScrollbarDirectives!: QueryList<QbsScrollbarDirective>;
    private _qbsScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any {
        /* eslint-disable @typescript-eslint/naming-convention */
        return {
            'qbs-vertical-navigation-animations-enabled':
                this._animationsEnabled,
            [`qbs-vertical-navigation-appearance-${this.appearance}`]: true,
            'qbs-vertical-navigation-hover': this._hovered,
            'qbs-vertical-navigation-inner': this.inner,
            'qbs-vertical-navigation-mode-over': this.mode === 'over',
            'qbs-vertical-navigation-mode-side': this.mode === 'side',
            'qbs-vertical-navigation-opened': this.opened,
            'qbs-vertical-navigation-position-left': this.position === 'left',
            'qbs-vertical-navigation-position-right':
                this.position === 'right',
        };
        /* eslint-enable @typescript-eslint/naming-convention */
    }
    ...
    /**
     * Setter for qbsScrollbarDirectives
     */
    @ViewChildren(QbsScrollbarDirective)
    set qbsScrollbarDirectives(
        qbsScrollbarDirectives: QueryList<QbsScrollbarDirective>
    ) {
        // Store the directives
        this._qbsScrollbarDirectives = qbsScrollbarDirectives;

        // Return if there are no directives
        if (qbsScrollbarDirectives.length === 0) {
            return;
        }

        // Unsubscribe the previous subscriptions
        if (this._qbsScrollbarDirectivesSubscription) {
            this._qbsScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._qbsScrollbarDirectivesSubscription = merge(
            ...
            .subscribe(() => {
                ...
                qbsScrollbarDirectives.forEach((qbsScrollbarDirective) => {
                    qbsScrollbarDirective.update();
                });
            });
    }
    ...
    ngAfterViewInit(): void {
        ...
        setTimeout(() => {
            ...
            if (
                ...
                const activeItem =
                    this._navigationContentEl.nativeElement.querySelector(
                        '.qbs-vertical-navigation-item-active'
                    );
            ...
            // Otherwise
                ...
                this._qbsScrollbarDirectives.forEach(
                    (qbsScrollbarDirective) => {
                        // Skip if not enabled
                        if (!qbsScrollbarDirective.isEnabled()) {
                            return;
                        }

                        // Scroll to the active element
                        qbsScrollbarDirective.scrollToElement(
                            '.qbs-vertical-navigation-item-active',
                            -120,
                            true
                        );
     ...
     * @param item
     */
    openAside(item: QbsNavigationItem): void {
     ...
     * @param item
     */
    toggleAside(item: QbsNavigationItem): void {
        // Toggle
    ...
    private _showOverlay(): void {
        ...
        this._overlay.classList.add('qbs-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if (this.transparentOverlay) {
            this._overlay.classList.add(
                'qbs-vertical-navigation-overlay-transparent'
            );
        }
    ...
    private _showAsideOverlay(): void {
        ...
        this._asideOverlay.classList.add(
            'qbs-vertical-navigation-aside-overlay'
        );
