...
import { QbsDrawerService } from '../drawer/drawer.service';
import {
    QbsDrawerMode,
    QbsDrawerPosition,
} from './drawer.types';
...
export class QbsDrawerComponent implements OnChanges, OnInit, OnDestroy {
    ...
    private _qbsDrawerService = inject(QbsDrawerService);
    private _qbsUtilsService = inject(QbsUtilsService);

    @Input() fixed: boolean = false;
    @Input() mode: QbsDrawerMode = 'side';
    @Input() name: string = this._qbsUtilsService.randomId();
    @Input() opened: boolean = false;
    @Input() position: QbsDrawerPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output() readonly fixedChanged: EventEmitter<boolean> =
        new EventEmitter<boolean>();
    @Output() readonly modeChanged: EventEmitter<QbsDrawerMode> =
        new EventEmitter<QbsDrawerMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> =
        new EventEmitter<boolean>();
    @Output() readonly positionChanged: EventEmitter<QbsDrawerPosition> =
        new EventEmitter<QbsDrawerPosition>();
    ...
    @HostBinding('class') get classList(): any {
        /* eslint-disable @typescript-eslint/naming-convention */
        return {
            'qbs-drawer-animations-enabled': this._animationsEnabled,
            'qbs-drawer-fixed': this.fixed,
            'qbs-drawer-hover': this._hovered,
            [`qbs-drawer-mode-${this.mode}`]: true,
            'qbs-drawer-opened': this.opened,
            [`qbs-drawer-position-${this.position}`]: true,
        };
        /* eslint-enable @typescript-eslint/naming-convention */
    }
    ...
    ngOnInit(): void {
        // Register the drawer
        this._qbsDrawerService.registerComponent(this.name, this);
    }
    ...
    ngOnDestroy(): void {
        ...
        this._qbsDrawerService.deregisterComponent(this.name);
    }
    ...
    private _showOverlay(): void {
        ...
        this._overlay.classList.add('qbs-drawer-overlay');

        // Add a class depending on the fixed option
        if (this.fixed) {
            this._overlay.classList.add('qbs-drawer-overlay-fixed');
        }

        // Add a class depending on the transparentOverlay option
        if (this.transparentOverlay) {
            this._overlay.classList.add('qbs-drawer-overlay-transparent');
        }
