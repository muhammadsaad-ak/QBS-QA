...
import { QbsConfigService } from '../../services/config';
import { fromPairs } from 'lodash-es';
import { Observable, ReplaySubject, map, switchMap } from 'rxjs';

@Injectable({ providedIn: 'root' })
export class QbsMediaWatcherService {
    private _breakpointObserver = inject(BreakpointObserver);
    private _qbsConfigService = inject(QbsConfigService);
                        ...
                        .pipe(
                            map((state) => {
                                ...
                                for (const [query] of matchingBreakpoints) {
                                    ...
                                    const matchingAlias = Object.entries(screens).find(([alias, q]) => q === query)?.[0] ?? null;
