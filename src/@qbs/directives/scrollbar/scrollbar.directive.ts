 ...
 */
@Directive({
    selector: '[qbsScrollbar]',
    exportAs: 'qbsScrollbar',
    standalone: true,
})
export class QbsScrollbarDirective implements OnChanges, OnInit, OnDestroy {
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_qbsScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    private _elementRef = inject(ElementRef);
    private _platform = inject(Platform);

    @Input() qbsScrollbar: boolean = true;
    @Input() qbsScrollbarOptions: PerfectScrollbar.Options | undefined;
    ...
    ngOnChanges(changes: SimpleChanges): void {
        // Enabled
        if ('qbsScrollbar' in changes) {
            // Interpret empty string as 'true'
            this.qbsScrollbar = coerceBooleanProperty(
                changes.qbsScrollbar.currentValue
            );

            // If enabled, init the directive
            if (this.qbsScrollbar) {
                this._init();
            }
        ...
        // Scrollbar options
        if ('qbsScrollbarOptions' in changes) {
            // Merge the options
            this._options = merge(
                {},
                this._options,
                changes.qbsScrollbarOptions.currentValue
            );
