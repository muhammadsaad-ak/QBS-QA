...
import { QBS_MOCK_API_DEFAULT_DELAY } from './mock-api.constants';
import { QbsMockApiService } from './mock-api.service';
...
export const mockApiInterceptor = (
    ...
    const defaultDelay = inject(QBS_MOCK_API_DEFAULT_DELAY);
    const qbsMockApiService = inject(QbsMockApiService);

    // Try to get the request handler
    const { handler, urlParams } = qbsMockApiService.findHandler(
