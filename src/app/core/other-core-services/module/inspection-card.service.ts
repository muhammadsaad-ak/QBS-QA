...
export class InspectionCardService {
    ...
    inspectionCardModal$: Observable<any[]> =
        this._inspectionCardModal.asObservable(); // IC API
    inspectionCardCode$: Observable<any[]> =
        this._inspectionCardsCode.asObservable(); //For IC Code API
    inspectioncard$: Observable<any[]> = this._inspectionCard.asObservable(); // IC API
    inspectioncardUpdate$: Observable<any[]> =
        this._inspectionCardUpdate.asObservable(); // IC API
    ...
    getInspectionCardCode(): Observable<any> {
        ...
        return this._httpClient
            .get(
                `${environment.appApiUrl}/CSAPI/INextIntCodeFeature/GetNextIntCount?entityName=inspection_card`,
                { headers }
            )
            .pipe(
                tap((inspectionCardCode) => {
                    const inspectionCardsCode =
                        (inspectionCardCode as any) ?? [];
                    this._inspectionCardsCode.next(inspectionCardsCode);
                    console.log(
                        'Fetched code of Inspection Code',
                        inspectionCardsCode
                    );
                }),
                catchError((error) => {
    ...
    getInspectionCardModal(): Observable<any> {
        ...
        return this._httpClient
            .get(
                `${environment.appApiUrl}/CSAPI/IInspectionCharacteristicFeature/ListAllInspectionCharacteristics`,
                { headers }
            )
            .pipe(
                tap((inspectionCardsModal) => {
                    const inspectionCardModal =
                        (inspectionCardsModal as any) ?? [];
                    this._inspectionCardModal.next(inspectionCardModal);
                    console.log(
                        'Fetched list of Inspection Card Modal',
                        inspectionCardModal
                    );
                }),
                catchError((error) => {
    ...
    getInspectionCard(): Observable<any> {
        ...
        return this._httpClient
            .get(
                `${environment.appApiUrl}/CSAPI/IInspectionCardFeature/ListAllInspectionCards`,
                { headers }
            )
            .pipe(
                tap((inspectionCard) => {
                    ...
                    console.log(
                        'Fetched list of Inspection Card',
                        inspectionCards
                    );
                }),
                catchError((error) => {
    ...
    UpdateInspectionCard(data: any): Observable<any> {
        ...
        return this._httpClient
            ...
            .pipe(
                ...
                catchError((error) => {
                    console.error(
                        'Error while updating Inspection Card Result',
                        error
                    );
    ...
    //Get list of Qualitative and Quantitative Criteria
    getCharacteristicsByInspectionCardId(inspectionCardId: string): Observable<any> {
        const headers = new HttpHeaders({
            Authorization: `Bearer ${this.accessToken}`,
        ...
        return this._httpClient
            .get(`${environment.appApiUrl}/CSAPI/IInspectionCardFeature/ListCharacteristicsByInspectionCardId?inspectionCardId=${inspectionCardId}`, { headers })
            .pipe(
                tap((res) => {
                    console.log('Fetched Characteristics:', res);
                }),
                catchError((error) => {
                    console.error('Error fetching characteristics:', error);
                    return throwError(() => new Error('Error fetching characteristics'));
                })
            );
    }
